...
export function filename(filePath: string, ext: boolean = true): string {
  ...
  let base =
    lastSlashIndex === -1 ? normalized : normalized.slice(lastSlashIndex + 1);
...
export function getRelativePath(filePath: string, root: string): string {
  ...
  for (
    ;
    commonLength < Math.min(fileSegments.length, rootSegments.length);
    commonLength++
  ) {
  ...
  const relativeParts = Array(upSegments)
    .fill("..")
    .concat(fileSegments.slice(commonLength));
